...
\centering
	\begin{tikzpicture}
		...
		legend entries={$\ln(\frac{1 - \x}{\x})$,
			${\deltaGMin = -118, \gamma = 1,\Ne = 10^{4}}$,
			${\deltaGMin = -118, \gamma = 1,\Ne = 10^{5}}$,
			${\deltaGMin = -16.7, \gamma = 0.1,\Ne = 10^{4}}$,
			${\deltaGMin = -16.7, \gamma = 0.1,\Ne = 10^{5}}$,
		},
...
\begin{figure}[H]
	...
	axis x line=bottom,
	axis y line=left,
	legend entries={$f(\x) = \frac{1}{1 + \e^{\beta(\deltaGMin + \Nsite \gamma \x)}}$},
